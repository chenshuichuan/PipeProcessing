...
//		startPage();
    ...
    public String edit(@PathVariable("deptId") Integer deptId, ModelMap mmap) {
        ...
        Workplace workplace1 = workplaceService.selectWorkplaceById(workplace.getParentId());
        workplace.setParentName(workplace1.getWorkplaceName());
     ...
     * 加载派工工位结构列表树
     */
    @GetMapping("/arrangeTreeData")
    @ResponseBody
    public List<Map<String, Object>> arrangeTreeData(@RequestParam("parentId")Integer parentId) {
        Workplace workplace = new Workplace();
        workplace.setParentId(parentId);
        return workplaceService.selectArrangeTree(workplace);
    }

    /**
     * 选择派工工位结构树
     */
    @GetMapping("/selectArrangeTree/{workplaceId}")
    public String selectArrangeTree(@PathVariable("workplaceId") Integer deptId, ModelMap mmap) {
        mmap.put("workplace", workplaceService.selectWorkplaceById(deptId));
        return prefix + "/arrangeTree";
    }
}
