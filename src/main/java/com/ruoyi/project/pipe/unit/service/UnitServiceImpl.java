...
public class UnitServiceImpl implements IUnitService {
  ...
  private Order queryOrder(Unit unit, PipCutting pipCutting, WorkPipe workPipe) {
    ...
    int hasSurfaceTreat = 1;
    int hasNoInstall = 0;
    ///这里注意 要修改的
    if (unit.getName().contains("SW")) {
      unitHasF = 1;
    }
    if (pipCutting.getPipeShape().contains("弯")) {
      pipeShapeHasBend = 1;
    }
    if ("N".equals(workPipe.getOutfield())) {
      hasSurfaceTreat = 0;
    }
    if ("▲".equals(pipCutting.getNoInstalled())) {
      hasSurfaceTreat = 1;
    }
    Order order = orderRepository.findByUnitHasFAndPipeShapeHasBendAndHasSurfaceTreatAndHasNoInstall(unitHasF,
            pipeShapeHasBend, hasSurfaceTreat,hasNoInstall);
    return order;
  }
